...
/* ------------------- jasonwolfe: hybrid drag scroll -------------------
   Ported from my Madromys keymap. A quick tap of the drag-scroll key locks
   scrolling on (tap again to unlock); holding it past DS_HOLD_TERM makes it
   momentary, so it releases when the key does. Only applies when the A+ is
   in its stock "hold to activate" mode -- the control-layer option to switch
   to pure tap-toggle (user_config.drag_scroll_mode) still works as shipped. */
    ...
    switch (keycode) {
        ...
        case PKC_TGL_DRAG_SCRL:
            if (record->event.pressed) {
                ...
                /* Drop any hybrid state so the two modes can't disagree about
                   whether drag scroll is currently active. */
                ds_key_held         = false;
        ...
        case PKC_DRAG_SCROLL:
            /* Pure tap-to-toggle mode, selectable from the control layer. */
            if( user_config.drag_scroll_mode ) {
                if (record->event.pressed) {
                    toggle_drag_scroll();
                }
            }
            /* Hybrid mode (replaces the stock press-and-hold): tap to lock,
               hold for momentary. matrix_scan_user() promotes a held key into
               momentary mode once DS_HOLD_TERM has elapsed. */
            else {
                if (record->event.pressed) {
                    ds_key_held   = true;
                    ...
                    if (ds_momentary_active) {
                        ...
                        /* Released before DS_HOLD_TERM, so this was a tap. */
                        ds_locked      = !ds_locked;
...
/* Promote a held drag-scroll key into momentary mode. The A+ defines no
   matrix_scan_kb, so the weak default routes scans here. */
